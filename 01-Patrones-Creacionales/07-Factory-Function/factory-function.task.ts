...
function createLogger(level: LogLevel) {
  ...
  return (message: string) => {
    ...
    const logPrefix = { info: 'INFO', warn: 'WARNING', error: 'ERROR' };
    const logColor = { info: COLORS.white, warn: COLORS.yellow, error: COLORS.red };
    const log = `%c[${logPrefix[level]}: ${logDate}] ${message}`;
    console.log(log, logColor[level]);
    return log;
  }
}
